...
6. ~~**Content feature**~~ — long-form documents 1:1 attached to Talks. **PRs #385–#410 shipped 2026-05-24/25** (schema, shared rich-text module, Tiptap editor, autosave, propose tool + ProposalCard, replace, bulk, intent gate). **Direct-edit redesign shipped 2026-05-26** (PRs #416/#417/#418): the propose abstraction + ProposalCard were replaced with `apply_content_edit` + `content_edits` edit-log table — agent calls land as pending rows that the user accepts/rejects inline via the doc pane (Google Docs Suggested Edits model). Plans: `~/.claude/plans/silly-gathering-charm.md` (the redesign). Memory: `project_content_direct_edit_redesign.md` is the live source of truth; `project_content_feature.md` covers the original v1-v4 history.
7. **Jobs re-architecture (TODO, next week)** — Reports was killed (talk_outputs table dropped, Reports tab removed), so scheduled `talk_jobs` now always produce thread messages. The whole jobs surface needs rethinking in light of Content: should a scheduled job append to a Content doc via `apply_content_edit`? Should it post to the thread? Both? Job UI, deliverable concept, and scheduler semantics all open for redesign. No plan file yet — start fresh.
